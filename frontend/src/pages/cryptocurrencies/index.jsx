...
import CardCryptocurrencies from "@/components/cardCryptocurrencies/CardCryptocurrencies";
      ...
      <Layout>
        <div className="w-[98%] mx-auto  my-10   ">
          ...
          <div className="grid gap-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-3  ">
            {filter.map((crypto) => (
              <CardCryptocurrencies key={crypto._id} crypto={crypto} />
            ))}
          </div>
        </div>
      </Layout>
...
export async function getStaticProps() {
  ...
  const filter = data.filter((f) => f.category === "Criptomonedas");

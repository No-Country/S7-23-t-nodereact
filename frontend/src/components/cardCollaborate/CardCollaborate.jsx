...
const CardCollaborate = ({ collaborate }) => {
  return (
    <Link href={`/collaborate/${collaborate._id}`}>
      <div className="w-full   h-[570px] lg:w-[100%]   cursor-pointer   border-[2px] border-[#dfdada] rounded-[15px] shadow-2xl">
        ...
        <div className="flex flex-col mt-3 ">
          ...
          <div className="w-[93%]  flex justify-between px-[10px]  mt-6">
            ...
            <span className="font-bold text-2xl text-[#000000]  font-Manrope">
              {collaborate.parcialAmount === 0
                ? 0
                : Math.ceil(
                    (collaborate.parcialAmount * 100) / collaborate.totalAmount
                  )}
              %
            </span>
          </div>
              ...
              style={{
                width: `${
                  collaborate.parcialAmount === 0
                    ? 0
                    : Math.ceil(
                        (collaborate.parcialAmount * 100) /
                          collaborate.totalAmount
                      )
                }%`,
              }}
              className={`${
                collaborate.parcialAmount === 0 ? "h-0" : "h-[22px]"
